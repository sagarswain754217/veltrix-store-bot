...
def is_txid_already_used(txid: str) -> bool:
    """Prevent duplicate TXID / reference usage"""
    if not txid:
        return False
...
async def check_binance_deposit(txid_or_ref: str, expected_amount: float) -> dict:
    """
    Check Binance Deposit History using Read-Only API.
    Returns: {"found": True/False, "message": "..."}
    """
    try:
        ...
        if not isinstance(deposits, list):
            return {"found": False, "message": "Invalid response from Binance"}
        ...
        for dep in deposits:
            ...
            if status == 1 and search:
                if (search in tx_id) or (search == tx_id):
                    if abs(amount - expected_amount) <= 0.05 or amount >= expected_amount:
                        return {
                            "found": True,
                            "message": f"Deposit found: {amount} USDT",
                            "amount": amount
                        }

        return {"found": False, "message": "Transaction not found in recent Binance deposits"}

    except Exception as e:
        return {"found": False, "message": f"Error checking Binance: {str(e)}"}
...
async def handle_payment_proof(update, context):
    ...
    if waiting not in ["txid", "binance_ref"]:
        return
    ...
    if not order_id:
        await update.message.reply_text("❌ Session expired. Please start the order again.")
        context.user_data.clear()
        return
    ...
    if is_txid_already_used(proof):
        await update.message.reply_text(
            "❌ <b>This Transaction ID / Reference has already been used.</b>\n\n"
    ...
    if verification.get("found"):
        ...
        if len(stocks) < qty:
            conn.close()
            await update.message.reply_text(
                "✅ Payment detected, but not enough stock right now.\n"
                "Admin will deliver it shortly."
            )
            context.user_data.clear()
            return

        c.execute("UPDATE orders SET status = 'DELIVERED', approved_at = ?, delivered_at = ? WHERE order_id = ?",
    ...
    # ===== AUTO CHECK FAILED → KEEP PENDING =====
    conn.close()
    await update.message.reply_text(
        ...
        f"You will receive the product after approval.",
        parse_mode=ParseMode.HTML
    )
    context.user_data.clear()
...
async def handle_deposit_proof(update, context):
    waiting = context.user_data.get('waiting_for')
    if waiting not in ["deposit_txid", "deposit_binance_ref"]:
        return

    user_id = update.effective_user.id
    proof = update.message.text.strip()
    deposit_id = context.user_data.get('current_deposit_id')
    amount = context.user_data.get('deposit_amount')

    if not deposit_id or amount is None:
    ...
    # 2. Try automatic verification
    verification = await check_binance_deposit(proof, expected_amount)

    if verification.get("found"):
        # ===== AUTO APPROVE DEPOSIT =====
        c.execute("UPDATE users SET balance = balance + ? WHERE user_id = ?", (expected_amount, user_id))
        c.execute("UPDATE wallet_deposits SET status = 'APPROVED', approved_at = ? WHERE deposit_id = ?",
    ...
    conn.close()
    await update.message.reply_text(
        f"⏳ <b>Deposit Proof Received</b>\n\n"
        f"🆔 Deposit ID: <code>{deposit_id}</code>\n"
        f"💰 Amount: <b>{format_money(expected_amount)}</b>\n"
        f"📄 Proof: <code>{proof}</code>\n\n"
        f"Status: <b>Pending Admin Approval</b>\n"
        f"Balance will be credited after approval.",
        parse_mode=ParseMode.HTML
    )
    context.user_data.clear()
...
async def admin_users(update, context):
    ...
    text = "👥 <b>Manage Users</b> (Top 30 by spending)\n\nSelect a user:"
    keyboard = []
    for u in users:
        name = u['full_name'] or "No Name"
        uname = f"@{u['username']}" if u['username'] else "No Username"
        btn_text = f"{name} | {uname} | 💰{format_money(u['balance'])}"
        keyboard.append([InlineKeyboardButton(btn_text[:60], callback_data=f"view_user_{u['user_id']}")])
...
async def add_balance_start(update, context):
    ...
    await query.edit_message_text(
        ...
        f"Please send the amount you want to add (example: 5 or 10.5):",
        parse_mode=ParseMode.HTML
    )
...
async def remove_balance_start(update, context):
    ...
    await query.edit_message_text(
        ...
        f"User ID: <code>{user_id}</code>\n\n"
        f"Please send the amount you want to remove (example: 5 or 10.5):",
        parse_mode=ParseMode.HTML
    )
...
async def handle_admin_balance_amount(update, context):
    ...
    try:
        ...
        await update.message.reply_text("❌ Please send a valid number (example: 5 or 12.5)")
        return
    ...
    if action == "add_balance":
        ...
        action_text = f"added <b>{format_money(amount)}</b> to"
    else:
        c.execute("UPDATE users SET balance = MAX(balance - ?, 0) WHERE user_id = ?", (amount, user_id))
        action_text = f"removed <b>{format_money(amount)}</b> from"
    ...
    await update.message.reply_text(
        f"✅ Successfully {action_text} user <code>{user_id}</code>\n\n"
        f"New Balance: <b>{format_money(new_balance)}</b>",
        parse_mode=ParseMode.HTML
    )
...
async def message_router(update, context):
    ...
    elif context.user_data.get('waiting_for') == "admin_balance_amount":
