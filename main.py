...
async def approve_deposit(update, context):
    ...
    if not deposit:
        ...
        await query.answer("Already processed.", show_alert=True)
        return
    c.execute("UPDATE users SET balance = balance + ? WHERE user_id = ?", (deposit['amount'], deposit['user_id']))
    c.execute("UPDATE wallet_deposits SET status = 'APPROVED', approved_at = ? WHERE deposit_id = ?", (get_current_time(), deposit_id))
    conn.commit()
    conn.close()
    try:
        await context.bot.send_message(chat_id=deposit['user_id'], text=f"✅ Deposit Approved!\n💰 +{format_money(deposit['amount'])}", parse_mode=ParseMode.HTML)
    except:
        pass
